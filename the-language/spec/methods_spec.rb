...
RSpec.describe "methods in ruby" do
  ...
  it "is not possible to call methods with the wrong number of arguments" do
    ...
    expect { a_global_method(1,2,3) }.to raise_error( ArgumentError, /wrong/ ) # HINT: Fill in the error and part of the message
  end
  ...
  class MyClass
    ...
    def calls_the_protected_method
      the_protected_method
    end

    def calls_the_protected_method_on_self
      self.the_protected_method
    end
  ...
  context "methods on objects" do
    ...
    it "does something if a protected method has an explicit receiver" do
      expect { an_object.the_protected_method}.to raise_error( NoMethodError,/protected/)
    end

    it "does something if a protected method has an explicit receiver" do
      expect( an_object.calls_the_protected_method_on_self).to eq(:the_protected_value)
    end
